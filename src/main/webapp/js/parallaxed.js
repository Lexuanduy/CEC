...
(function($) {
	...
	$.fn[namespace] = function(options) {
		...
		// Internal helper function to get data items, falling back to the
		// default
						...
						function() {
							...
							$items
									.each(function(index, item) {
										...
										var start = getDataDefault($item,
												'start');
										var speed = getDataDefault($item,
												'speed');
										...
										var parallaxOffsetTop = offsetTop
												* speed;
										var parallaxOffsetLeft = offsetLeft
												* speed;

										// Check if we should be moving
										if (start == 'visible') {
											if ($item.position().top > offsetTop
													+ $window.height()
													|| startTop
															+ $item
																	.outerHeight() < offsetTop
													|| $item.position().left > offsetLeft
															+ $window.width()
													|| startLeft
															+ $item
											...
											// Have to adjust offset based on
											// how much had to be scrolled to
											// see the item
											var scrollToSeeTop = (startTop > $window
													.height()) ? startTop
													- $window.height() : 0;
											parallaxOffsetTop = (offsetTop - scrollToSeeTop)
													* speed;

											var scrollToSeeLeft = (startLeft > $window
													.width()) ? startLeft
													- $window.width() : 0;
											parallaxOffsetLeft = (offsetLeft - scrollToSeeLeft)
													* speed;
										} else if (start == 'offset') {
											var startOffsetTop = getDataDefault(
													$item, 'startoffsettop');
											var startOffsetLeft = getDataDefault(
													$item, 'startoffsetleft');
											if (offsetTop < startOffsetTop
													|| offsetLeft < startOffsetLeft) {
												return;
											}

											// Adjust offset based on start
											// offset
											parallaxOffsetTop = (offsetTop - startOffsetTop)
													* speed;
											parallaxOffsetLeft = (offsetLeft - startOffsetLeft)
													* speed;
										}

										if (getDataDefault($item,
												'usetransform')) {
											...
											$item
													.css(
															'top',
															(startCssTop - parallaxOffsetTop)
																	+ "px");
											$item
													.css(
															'left',
															(startCssLeft - parallaxOffsetLeft)
																	+ "px");
										}
									});
						});
